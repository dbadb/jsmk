//
// 'vc12.js': a tool to invoke the visual c compiler..
// we assume that the controlling toolset (vs12) has made VSROOT available.
// This tool relies upon the environment to convey access to
// standard locations.
//
{
    "schema": "gomk/tool",
    "tooltype": "archiver",
    "invocation": {
        "o->a": "lib $FLAGS /OUT:$DSTFILE $SRCFILES"
    }
}
...
*/
//
// 'vc12.ini': a tool to invoke the visual c compiler..
// we assume that the controlling toolset (vs12) has made VSROOT available.
// This tool relies upon the environment to convey access to
// standard locations.
//
{
    ...
    "tooltype": "compiler",
    "invocation": {
        "cpp->o": "cl $SRCFILE /Fo:$DSTFILE $INCLUDES $DEFINES $FLAGS"
    },
    "syntax": {
      "define": "/D${KEY}=${VAL}",
      "include":  "/I${VAL}",
    ...
    "environment": {
      "INCLUDE": "${VSROOT}/VC/include;${VS_SDK}"
    },

    // vc12-specific defines go here, (not, eg, NDEBUG)
    "define": {
    },
    "define.debug": {
    },
...
/arch:<IA32|SSE|SSE2|AVX|AVX2> minimum CPU architecture requirements, one of:
   ...
   AVX2 - enable use of Intel(R) Advanced Vector Extensions 2 instructions
...
/Zo[-] generate richer debugging information for optimized code (off by default)/ZW enable WinRT language extensions    /ZI enable Edit and Continue debug info
...
//
{
    ...
    "tooltype": "linker",
    "invocation": {
        "c.o->exe": "link $FLAGS /OUT:$DSTFILE $SRCFILES",
        "cpp.o->exe": "link $FLAGS /OUT:$DSTFILE $SRCFILES"
    },
    "environment": {
      "LIB": "${VSROOT}/lib"
    }
