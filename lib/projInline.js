    ...
    let newProj = rootproj.NewProject(basename, 
    {
        ...
        init: function(subProj) 
        {
            ...
            for(let i in projlist)
            {
                ...
                if(!ldflags) ldflags = projlist[i].linkflags;
                ...
                let modifyInvoc = projlist[i].modifyInvoc; // usually undefined
                if(!deps)
                {
                ...
                // infer isApp, can't use frameworks (eg eigen) 
                // since they may provide include paths to library builds.
                let isApp = (deplibs || deps || syslibs);
                ...
                subProj.NewProject(projnm, 
                {
                    ...
                    init: function(subProj) 
                    {
                        ...
                        if(src.c || extraCfiles.length > 0)
                        {
                            ...
                            for(let i of src.c)
                            {
                                srcfiles = srcfiles.concat(subProj.Glob(i))
                            }
                            ...
                            if(srcfiles.length)
                            {
                                ...
                                jsmk.DEBUG(`${projnm} lib.c flags: ` +
                                            JSON.stringify(ccflags));
                                libInputs = libInputs.concat(
                                                tc.GetOutputs());
                            }
                            else
                            {
                                jsmk.WARNING(`project: ${projnm} no c src files: ${src.c} (${subProj.ProjectDir})`);
                            }
                        }
                        // lib.cpp ----------------------------
                        if(src.cpp)
                        ...
                        {
                            let libnm;
                            if(isApp && !projnm.startsWith("lib")) 
                                libnm = "lib" + projnm;
                            else
                                libnm = projnm;
                        ...
                        if(src.apprc && src.apprc.length)
                        {
                            let trc = m.NewTask(`${projnm}-meta`, "rc->o", {
                                inputs: src.apprc,
                                modifyInvoc
                            });
                            appInputs = appInputs.concat(trc.GetOutputs());
                            jsmk.DEBUG(`rc files for ${projnm} ${trc.GetOutputs()}`);
                        }
