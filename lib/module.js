...
class Module extends SettingsContainer
{
    ...
    *GenerateWork(stage)
    ...
    applyModules(task, primary)
    {
        ...
        if(this.m_clientconfig)
        {
            ...
            if(mods && mods.length > 0)
                jsmk.DEBUG(`${this.GetName()} has clientcfg module refs`);
        }
        if(!mods && this.ModuleList && this.ModuleList.length > 0)
        {
            jsmk.DEBUG(`${this.GetName()} has regular module refs`);
    ...
    configureTask(t)
    {
        ...
        if(this != t.GetModule())
        {
            jsmk.DEBUG(`${t.GetModName()} configured by ${modname} as ` +
        ...
        if(sp && sp.length)
        {
            ...
            sp = nsp;
            jsmk.DEBUG(`${modname}/${role} searchpaths: ${sp}`);
        ...
        case Tool.Role.Link:
            {
                ...
                if(deplibs && deplibs.length) 
                    ...
                    deplibs = "";
                jsmk.DEBUG(`${modname} lib:${lib} deplibs:${deplibs}`);
