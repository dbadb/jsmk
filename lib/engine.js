PromiseIteratorQueue = require("./promiseQueue");
class Engine
{
    constructor(stage, projTree, startdir)
    {
        this.m_stage = stage;
    ...
    //
    //
    Run(stage, threads, ondone)
    {
        try
        {
            let promiseQ = new PromiseIteratorQueue({
                            maxConcurrency: threads,
                            onDrain: ondone
                        });
            let projIterator = this.m_projTree.GetProjIterator("depthfirst");
            while(true)
            {
                let proj = projIterator.next();
                if(!proj) break;
                if(jsmk.path.issubdir(proj.ProjectDir, this.m_startDir))
                {
                    promiseQ.Append(proj.GetWorkIterator(stage)]);
                }
            }
        }
        catch(e)
