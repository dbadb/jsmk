...
class Jsmk
{
    ...
    EndSession(err)
    {
        this.TIME('jsmk end } ' + (err ? "(FAILURE)" : "(SUCCESS)"));
        this.m_toolsets = [];
    }
...
jsmk  [-b Policy (default: 'default')]
    ...
    LogSubProcess(invoc)
    {
        ...
        {
            istr = this.path.basenameNoExt(invoc[0]) + " ";
    ...
    msgFilter(msg)
    {
        if(this.m_build)
        {
            if(!this.m_projRootFilter)
            {
                ...
                if(pt)
                {
                    let dir = pt.ProjectDir;
                    this.m_projRootFilter = new RegExp(dir + "/", "g");
                }
            }
            if(this.m_projRootFilter)
                return msg.replace(this.m_projRootFilter, "");
        }
        else
