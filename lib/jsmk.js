...
class Jsmk
{
    constructor()
    {
        ...
        if(!process.env.HOME)
            process.env.HOME = this.m_user.HomeDir;
    ...
    GetTargetPlatform()
    {
        if(global.Toolset && global.Toolset.TargetPlatform)
            return global.Toolset.TargetPlatform; 
        else
            return this.GetHost().Platform;
    }
    ...
    {
        this.TIME("jsmk end } " + 
            (err ?  "<ERROR>(FAILURE)</ERROR>" : "<INFO>(SUCCESS)</INFO>"));
        this.m_toolsets = [];
    }
    ...
    doBuild(toolset)
    {
        ...
        return new Promise( function(resolve, reject) {
            self.TRACE("doBuild " + toolset.GetHandle());
    ...
    GetFramework(name, buildvars)
    {
        let vers = buildvars[`${name}_vers`] || "default";
        let key = `${name}_${vers}`;
        if(!this.m_frameworks[key])
        {
            ...
            try
            {
                FW = this.LoadConfig(`framework/${name}.js`, true).Framework;
            }
            catch(err)
            {
                ...
                try
                {
                    FW = this.LoadConfig(`framework/${platform}/${name}.js`, true).Framework;
                }
                catch(err)
                {}
            }
            if(!FW)
                this.ERROR("Can't find configfile: " + name);
            else
...
jsmk  [-b|-p Policy (default: 'default')]  
      [-x ts:clang (string passed as config.x to policy constructor)]
      [-r buildrootdir (default: '_inferred')]]
      [-t buildthreads (default: 0 (means use-all))]
      [-l level (TRACE|DEBUG|INFO|WARNING)]
      ...
      [buildstagelist | 'log' (default: '_deferToPolicy')]
        ...
        let config = {};
        let loglevel = "INFO";
        // minimize can't handle standalone -n followed by stages
        if(argmap.n)
        {
            if(argmap.n !== true)
            {
                jsmk.NOTICE("argmap.n value " + argmap.n);
                argmap._.push(argmap.n);
            }
        }
        for(let key in argmap)
        {
            ...
            case "r": // -r rootdir
                ...
                break;
            case "x": // of potential use by custom policy
                config.xtra = argmap.x;
                break;
            default:
                jsmk.WARNING(key);
                err = 1;
