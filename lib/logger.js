...
class Logger
{
    constructor(logdir)
    {
        ...
        this.logfile = new winston.transports.DailyRotateFile(
                        {
                            eol: "\n",
                            level: "TRACE",
                            format: this.fileFormatter,
                            datePattern: "MM-DD",
                            filename: path.join(logdir, "jsmk-%DATE%.log"),
                            maxFiles: "14d",
                        });
...
class CustomFormatter 
{
    ...
    fmtMsg(info)
    {
        ...
        {
            this.lastTime = now;
            ts = (this.fmtLevel("TIME") + info.fmtTime() + " ----------\n");
        }
        if(this.msgFilter)
