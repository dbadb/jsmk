...
class Task extends SettingsContainer
{
    constructor(name, rule, module, config)
    {
        ...
        let stashBuildVars = config.buildvars;
        if(stashBuildVars)
        {
        ...
        // calls this.MergeSettings(tool) which populates with tools values BuildVars
        if(stashBuildVars)
        {
    ...
    *GenerateWork(stage)
    {
        ...
        try
        {
            ...
            let tstage = this.m_actionStage || this.m_tool.GetActionStage();
