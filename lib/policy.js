...
exports.NewBuildPolicy = function(jsmk, host, user) // ---- factory -----
{
    return new BuildPolicy(jsmk, host, user);
}
...
function BuildPolicy(host, user) // ---- constructor ----
{
    ...
    this.InstallDirTmplt = "${ROOTDDIR}/_install";
    this.PackageDirTmplt = "${ROOTDDIR}/_package";
    this.ProjectMatch = "*";
    this.ToolsetMatch = "*";
}
...
{
    Init : function(rootdir, configdir, policy, stages, perform, threads)
    {
    ...
    loadPolicy : function(dir, nm)
    {
        ...
        var policy = require(file).BuildPolicy;
        for(i in policy)
        {
