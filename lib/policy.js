...
// Policy is pure data.. policy files export a signle var/map
...
class Policy
{
    ...
    Init(rootdir, configdirs, policy, stages, perform, threads)
    {
        this.loadPolicy(configdirs, policy); // should occur first (may throw)
    ...
    loadPolicy(dirlist, nm)
    {
        // policy files are expected  in "dir/policy/nm.js"
        let success = false;
        for (let dir of dirlist)
        {
            let file = util.FixupPath(path.join(dir, "policy", nm + ".js"));
            try
            {
                fs.accessSync(file);
                success = true;
        ...
        if(!success)
            throw("Unable to load policy file named: " + nm);
    }
}
