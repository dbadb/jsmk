...
class ProjDomain
{
    ...
    FindModule(nm)
    {
        ...
        if(!m)
        {
            jsmk.DEBUG(this.name + " can't find module " + nm);
...
class Project extends SettingsContainer
{
    ...
    constructor(name, parent, policy, config)
    {
        ...
        if(config.init)
        {
            jsmk.DEBUG(this.ProjectDir + " functional init() ");
            config.init(this);
        }
        else
        {
    ...
    FindModule(mnm)
    {
        ...
        if(!mod)
        {
            jsmk.WARNING(`${this.GetDomainName()} can't find module ${mnm}`);
        }
        return mod;
    }
            ...
            init: function(subProj) {
                ...
                for(let i in projlist)
                {
                    ...
                    subProj.NewProject(projnm, {
                        ProjectDir: projdir,
                        init: function(subProj) {
                            let m = subProj.NewModule(projnm);
                            let archiveObjs = [], appObjs = [];
                            if(src.cpp)
                            {
                                let srcfiles = [];
                            ...
                            if(archiveObjs.length > 0)
                            {
                                ...
                                if(syslibs)
                                    ...
                                    libnm = projnm;
                                let tar = m.NewTask(libnm, "o->a", {
                                    inputs: archiveObjs
                                });
                                configureTask(tar, modules);
                                if(syslibs)
                                    appObjs = tar.GetOutputs();
                            }
                            else
                            ...
                            if(src.appcpp)
                            {
                                ...
                                if(srcfiles.length)
                                {
                                    ...
                                    configureTask(tcpp, modules);
                                    appObjs = appObjs.concat(tcpp.GetOutputs());
                                }
                            }
                            if(src.appc)
                            {
                                ...
                                if(srcfiles.length)
                                {
                                    ...
                                    configureTask(tc, modules);
                                    appObjs = appObjs.concat(tc.GetOutputs());
                                }
                            }
                            if(appObjs.length > 0)
                            {
                                let libfiles = appObjs;
                                for(let i in libs)
                                    libfiles = libfiles.concat(rootproj.FindModuleOutputs(libs[i]));
                                if(syslibs)
                                    libfiles = libfiles.concat(syslibs);
