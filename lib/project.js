...
class Project extends SettingsContainer
{
    ...
    constructor(name, parent, policy, config)
    {
        ...
        {
            jsmk.DEBUG(name + " " + jsmk.path.relative(this.GetRootDir(), this.ProjectDir) + " init (functional)");
            config.init(this);
        }
        else
        {
                ...
                for(let i in projlist)
                {
                    ...
                    subProj.NewProject(projnm, {
                        ...
                        init: function(subProj) {
                            let modConfig;
                            if(clientcfg) 
                                modConfig = { clientconfig: clientcfg };
                            else
                                modConfig = {};
                            ...
                            if(!src.appcpp && !src.appc && !src.cpp && !src.c)
                            {
