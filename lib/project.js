    ...
    EstablishDomain(name, toolsetConstraints, config)
    {
        ...
        if(config)
        {
            ...
            if(config.TimestampFile)
            {
                ...
                jsmk.file.write(tsfile, JSON.stringify(ts, null, 2), (err) =>
                {
        ...
        let newProj = this.NewProject(basename, 
        {
            ...
            init: function(subProj) 
            {
                ...
                for(let i in projlist)
                {
                    ...
                    let isApp = (deplibs || syslibs || frameworks);
                    ...
                    subProj.NewProject(projnm, 
                    {
                        ...
                        init: function(subProj) 
                        {
                            ...
                            if(src.y && src.y.length)
                            {
                                ...
                                if(srcfiles.length)
                                {
                                    let ty = m.NewTask("compile_y", "y->c", 
                                    {
                                        inputs: srcfiles,
                                        searchpaths: inc,
                                        define: defs,
                            ...
                            if(src.lex)
                            {
                                ...
                                if(srcfiles.length)
                                {
                                    let tl = m.NewTask("compile_lex", "lex->c",
                                    {
                                        inputs: srcfiles,
                                        searchpaths: inc,
                                        define: defs,
                            ...
                            if(src.cpp)
                            {
                                ...
                                if(srcfiles.length)
                                {
                                    let tcpp = m.NewTask("compile_cpp", 
                                                        "cpp->o", 
                                    {
                                        inputs: srcfiles,
                                        flags: cppflags,
                            ...
                            if(src.appcpp || src.dllcpp)
                            {
                                ...
                                if(srcfiles.length)
                                {
                                    let tcpp = m.NewTask("compile_appcpp", 
                                                    "cpp->o", 
                                    {
                                        inputs: srcfiles,
                                        flags: cppflags,
                            ...
                            if(appInputs.length > 0)
                            {
                                ...
                                {
                                    deps = deps.concat(
                                        rootproj.FindModuleOutputs(deplibs[i]));
                                }
