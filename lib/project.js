        ...
        let newProj = this.NewProject(basename, {
            ...
            init: function(subProj) {
                ...
                for(let i in projlist)
                {
                    ...
                    // clientconfig: { // when archives configures a client
                    //  libs: 
                    //  syslibs: 
                    // }
                    let modules = projlist[i].modules;
                    let inlibs = projlist[i].libs;
                    let syslibs = projlist[i].syslibs;
                    let moduleconfig = projlist[i].moduleconfig;
                    let isApp = inlibs ? true : (syslibs ? true : false);

                    subProj.NewProject(projnm, {
                        ProjectDir: projdir,
                        init: function(subProj) {
                            let m = subProj.NewModule(projnm, moduleconfig);
                            ...
                            if(appInputs.length > 0)
                            {
                                ...
                                for(let i in inlibs)
                                    inputs = inputs.concat(rootproj.FindModuleOutputs(inlibs[i]));
