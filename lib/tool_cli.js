    ...
    *GenerateWork(stage, task, inputs, triggers, outputs)
    {
        ...
        case Tool.Semantics.NoneToNone:
            ...
            yield this.makeWork(task, invoc, cwd, null, tnm, envmap);
            break;
        ...
        case Tool.Semantics.ManyToOne:
            if(this.outputIsDirty(outputs[0], inputs.concat(triggers), cwd))
            {
                invoc = this.getInvocation(task, inputs, outputs);
                yield this.makeWork(task, invoc, cwd, outputs[0], tnm, envmap);
            }
            break;
        ...
        case Tool.Semantics.ManyToMany:
            for(let i=0;i<inputs.length;i++)
            {
                ...
                if(this.outputIsDirty(output, input.concat(triggers), cwd))
                {
                    invoc = this.getInvocation(task, [input], [output]);
                    let tnmt = `${tnm}_${i}/${inputs.length}`;
                    yield this.makeWork(task, invoc, cwd, output, tnmt,envmap);
                }
            }
            break;
        }
    ...
    makeWork(task, invoc, cwd, outfile, nm, envMap)
    {
        let self = this;
        let w  = new Promise( (resolve,reject) => {
            jsmk.path.normalizeArgv(invoc, cwd);
            ...
            if(envMap.INCLUDE)
            {
                ...
                jsmk.ERROR("no envMap.INCLUDE");

            let shorthand = `${task.GetTool().GetRole()} ` +
                `${jsmk.path.basename(outfile)} ` +
                `(${cwd})`;
