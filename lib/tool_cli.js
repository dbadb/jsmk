    ...
    *GenerateWork(doit, task, inputs, triggers, outputs)
    {
        ...
        let invoc;
        let envmap = Object.assign({}, process.env, task.EnvMap); // last one wins!
    ...
    getInvocation(task, inputs, outputs)
    {
        ...
        let newinvoc = [];
        task.ModifyInvoc(result, newinvoc);
        if(newinvoc.length == 0) return result;
        else return newinvoc;
    }
    ...
    makeWork(task, invoc, taskDir, outfile, nm, envMap)
    {
        ...
        let w  = new Promise( (resolve,reject) => {
            ...
            if(!envMap.PATH)
                envMap.PATH = process.env.PATH; // path is require by spawn
            if(envMap.INCLUDE)
            {
