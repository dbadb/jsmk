...
class Tool_Cli extends Tool
{
    ...
    *GenerateWork(stage, task, inputs, outputs)
        ...
        case Tool.Semantics.ManyToMany:
            for(let i=0;i<inputs.length;i++)
            {
                ...
                if(this.outputIsDirty(output, input, cwd))
                {
                    ...
                    yield this.makeWork(invoc, cwd, output, tnmt, task.EnvMap);
                }
            }
            break;
        }
    ...
    getInvocation(task, inputs, outputs)
    {
        ...
        {
            SRCFILE: inputs[0],
            SRCFILEBASENOEXT: jsmk.path.basenameNoExt(inputs[0]),
            DSTFILE: outputs[0],
        ...
        for(let i=0; i < arglist.length; i++)
        {
            ...
            case "${INCLUDES}":
                if(!this.m_syntax.Include)
                    throw new Error(`${this.m_name} has no syntax for Include`);
            ...
            this.childProcess =
                execFile(invoc[0], invoc.slice(1), {
                    ...
                    function(errcode, stderr, stdout)
                    {
                        if(stderr.length)
                            console.log("\nstderr:",
                                    jsmk.colors.apply('lightbrown',  stderr));
                        if(stdout.length)
                            console.log("\nstdout:",
                                    jsmk.colors.apply('brown',  stdout));
