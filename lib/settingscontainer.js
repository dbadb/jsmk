...
class SettingsContainer
{
    ...
    Configure(handle, role, config)
    {
       // we allow additional custom vals to be configured via _Proj
        ...
        if(config.searchpaths)
        {
            ...
            {
                for(let role in Object.keys(config.searchpaths))
        ...
        if(config.flags)
        {
            ...
            else
            {
                ...
                {
                    this.AddTypedFlags(key, config.flags[key]);
                }
            }
        }
        ...
        {
            jsmk.NOTICE(`${handle} has a toolconfig`);
    ...
    AddFlags(flist)
    {
        this.AddTypedFlags("_notype", flist);
    }

    // return an arglist of flags that obey the provided syntax
    //  since Flags are a list, we substitute ${VAL}.
    GetFlags(syntax)
    {
        return this.GetTypedFlags("_notype", syntax);
    }

    AddTypedFlags(type, flist)
    {
